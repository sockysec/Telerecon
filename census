...
def scrape_subscriber_counts(channel_urls):
    ...
    for url in channel_urls:
        ...
        subscriber_count_element = soup.find('div', class_='tgme_page_extra')
        if subscriber_count_element:
            ...
            subscriber_count = ''.join(c for c in member_count if c.isdigit())
            if subscriber_count:
                print(f"Channel: {Fore.BLUE + url + Style.RESET_ALL} | Members: {Fore.GREEN + subscriber_count + Style.RESET_ALL}")
    ...
    print(f"{Fore.RED}Please Note: This total does not account for duplication. A single user may belong to multiple channels and therefore be included multiple times within this count.{Style.RESET_ALL}")
